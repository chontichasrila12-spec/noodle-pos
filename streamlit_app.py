...
# ฟังก์ชันเชื่อมต่อ Google Sheets ผ่านกุญแจ gspread_credentials ที่คุณตั้งไว้ใน Secrets
def get_google_sheet():
    try:
        ...
        # ล็อกอินเข้าระบบด้วยกุญแจบอท
        gc = gspread.authorize(creds)
        # เชื่อมตรงเข้าไฟล์ชีตของคุณผ่าน ID ไฟล์
        spreadsheet_key = "1wh7K03kwanBF4fEP1SnKjHBJIHSJK-aMQPXGjQBdW1E"
        worksheet = gc.sheet_by_key(spreadsheet_key).worksheet("orders")
        return worksheet
    except Exception as e:
        st.error(f"🔧 บอทเข้าชีตไม่ได้เนื่องจาก: {e}")
        return None

worksheet = get_google_sheet()

# ดึงข้อมูลเก่ามาแสดงในหน้าสถิติ
if worksheet:
    try:
...
st.title("🍜 ระบบจัดการร้านก๋วยจั๊บ (เชื่อมต่อสำเร็จแล้ว)")
    ...
    with col2:
        ...
        if st.button("➕ บันทึกออเดอร์"):
            if worksheet:
                ...
                st.success("🎉 ออเดอร์ซิงค์เข้าระบบ Google Sheets สำเร็จ!")
                st.rerun()
            else:
                st.error("ไม่สามารถบันทึกได้ กรุณารีเฟรชหน้าจออีกครั้ง")
